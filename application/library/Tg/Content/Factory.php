...
/**
 * Tg Content Factory Class 
 */


class Tg_Content_Factory {
	 ...
	 * @param $contentId
	 * @param $version
	 * @return Tg_Content_Db_Content
	 */
	
	public static function getContent ($contentName, $version = 0)
	{
		$table = new Tg_Content_Db_Table_Content();
		$select = $table->select()
		...
		$contentRecord = $table->fetchRow($select);

		
		if (empty($contentRecord))
		{
			$contentRecord = $table->createRow();
			$contentRecord->name = $contentName;
			$contentRecord->version = 1;
			$contentRecord->data = '<data></data>';
			$contentRecord->created_at = new Zend_Date();
			$contentRecord->updated_at = new Zend_Date();
		}
    ...
    // remove once all content moved to to content
	 ...
	 * @return Tg_Content_Db_Content
	 */
    public static function getPageContent ($contentId, $contentVersion = 0)
    {
    	//$contentId = $this->_getParam('contentId');
    	$content = self::getContent($contentId, $contentVersion);
	...
	public static function getContentVersions ($contentName)
	{
		$table = new Tg_Content_Db_Table_Content();
		$select = $table->select()
			->where ('name=?',$contentName)
		...
		$contentRecords = $table->fetchAll($select);
		...
		foreach ($contentRecords as $contentRecord)
		{
			...
			$version->version = $contentRecord->version;
			$version->created_at = $contentRecord->created_at->get(Zend_Date::ISO_8601);
			$version->updated_at = $contentRecord->updated_at->get(Zend_Date::ISO_8601);
			$versions[] = $version;
		}
	...
	public static function getLastVersion ($contentName)
	{
		$table = new Tg_Content_Db_Table_Content();
		$select = 'SELECT MAX(version) FROM '.$table->_name.' WHERE name=\''.$contentName.'\'';
	...
	// $mode == 1 - Create a new version fo every save
	// $mode == 2 - Update last unpublished version
	public static function saveContent ($contentName, $contentData, $mode = 1)
	{
		$table = new Tg_Content_Db_Table_Content();
		
		if (empty($contentData))
			throw new Exception ('Critical error - no content found');
		
		if (trim($contentData) == '' || trim($contentData) == null)
			throw new Exception ('Critical error - content was blank');
			
	   $contentData = '<?xml version="1.0"?>'.stripslashes($contentData);
	   $version = self::getLastVersion($contentName);
	   ...
	   if ($mode == 1)
	   {
			...
			$contentRecord = $table->createRow();
			$contentRecord->name = $contentName;
			$contentRecord->version = $version+1;
			$contentRecord->data = $contentData;
			$contentRecord->created_at = new Zend_Date();
			$contentRecord->updated_at = new Zend_Date();
	   		$contentRecord->save ();
	   ...
	   {
	   	 throw new Exception ('Tg_Content::saveContent - mode 2 no implemented yet');
	   }
	}
		
	public static function saveXml ($data, $formXmlFile)
	{
		return self::getInstance()->_saveXml ($data, $formXmlFile);
	}
	
	public function _saveXml ($dataArray, $formXmlFile)
	{
		$formXml = simplexml_load_file($this->_formPath.'/'.$formXmlFile);
		
		$dataXml = new SimpleXMLElement ('<data></data>');
		
		$this->_saveXmlRec($dataArray, $formXml, $dataXml);
		
		return $dataXml;
	}
	
//	public function _parsePost ($type)
//	{
//		$elementClassName = "Tg_Content_Element_".ucfirst($type);
//		$xml = $elementClassName::getXml($xml);
//	}
	
//	public function _saveXmlRec ($dataArray, $formXml, $dataXml) {
//		if ($formXml->children ()) {
//			foreach ($formXml->children () as $formXmlChild) {
//				$type = (string)$formXmlChild->getName();
//				$id = (string)$formXmlChild->attributes()->id;
//				
//				if (!empty($dataArray[$id])) 
//				{
//					$data = $dataArray[$id];
//					
//					self::_parsePost ($type, $data);
//					
//					
//					dump ($data);
//					if (is_string($data))
...
//					{
//						case "group":
//							$dataXmlChild = $dataXml->addChild($type);
//							$dataXmlChild->addAttribute ('id', $id);
//							$fieldData = $dataArray[$id];
//							// $data[$id] is an array of option elements
//							
//							foreach ($fieldData as $key => $value) {
//								$optionId = array_keys($fieldData[$key]);
//								$optionId = $optionId[0];
//								$optionFormXml = $formXmlChild->xpath("option[@id='".$optionId."']");
//								$optionDataXml = $dataXmlChild->addChild('option');
//								$optionDataXml->addAttribute ('id', $optionId);
//								$this->_saveXmlRec($fieldData[$key][$optionId], $optionFormXml[0], $optionDataXml);
//							}
//							break;
//						case "file":
//							$file = Delete_File::getFile ($dataArray[$id]);
//							$dataXmlChild = $dataXml->addChild($type, $dataArray[$id]);
//							$dataXmlChild->addAttribute ('id', $id);
//							$dataXmlChild->addAttribute ('name', $file->name);
//							$dataXmlChild->addAttribute ('url', $file->getUrl('80x80'));
//							break;
//						case "html":
//							$dataXmlChild = $dataXml->addChild($type);
//							$dataXmlChild->addAttribute ('id', $id);
//							
//							$node = dom_import_simplexml($dataXmlChild);
...
//						default:
//							$dataXmlChild = $dataXml->addChild($type, $data);
//							$dataXmlChild->addAttribute ('id', $id);
//					}
//				}		
//			}
...
//	}
	...
	static function loadXmlFromFile ($xmlFileName) 
	{	
		return self::getInstance()->_loadXml($xmlFileName);
	}

	function _loadXmlFromFile ($xmlFileName) 
	{
		$dataXml = simplexml_load_file($this->_dataPath.'/'.$xmlFileName);
		
		return new Tg_Content_Data ($dataXml);
	}
	
	/**
	 * 
	 * Enter description here ...
	 * @param unknown_type $dataXmlString
	 * @return Tg_Content_Data $data
	 */
	
	static function loadXml ($dataXmlString) 
	{
		if (!empty($dataXmlString)){
			try {				
				$dataXml = simplexml_load_string($dataXmlString);
				
				if ($dataXml)
					return new Tg_Content_Data ($dataXml);
//				else 
//					dump ($dataXmlString);die;
				// TODO - Should this return a Tg_Content_Data with an error message?
			} catch (Exception $e)
			{
		...
		return new Tg_Content_Data (null);
	}
	
	public function getFormFromFile ($formXmlFile, $dataXml = '', $options = array())
	{
		$formXmlFile = $this->_formPath.'/'.$formXmlFile;
		if (is_file($formXmlFile)) {
			$formXml = file_get_contents($formXmlFile);
			
			return $this->getForm ($formXml, $dataXml, $options);
		} else
			return 'Form file ('.$formXmlFile.') not found';
	}
	...
	public function getForm ($formXml, $dataXml = '', $options = array())
	{
		...
		$formXml = $this->_stripWhiteSpace($formXml);
		
		if (empty($dataXml))
			$dataXml = '<?xml version="1.0"?><data></data>';
		
		$dataXml = $this->_stripWhiteSpace($dataXml);
		...
		$output ='
		<div id="'.substr($options['container'],1).'">Loading</div>
		<script>
		$(document).ready (function () {
		var formOptions = '.Zend_Json::encode ($options).';
		window.form = new CMS.Form (formOptions);
		form.loadXml (\''.escape($formXml).'\');
		form.populateXml (\''.escape($dataXml).'\');
		});
		</script>';
	...
	public function getFormObject ($formXmlFile, $dataXml = '', $options = array())
	{
		$json = new stdClass ();
...
//		$formXmlFile = $this->_formPath.'/'.$formXmlFile;
		if (is_file($formXmlFile)) {
			...
			throw new Exception('Form file ('.$formXmlFile.') not found');
		
		if (empty($formXml))
			throw new Exception('Form xml is not set');
	...
	public function _stripWhiteSpace($data) 
	{
